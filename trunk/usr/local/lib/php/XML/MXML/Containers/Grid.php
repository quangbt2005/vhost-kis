 ...
 * @author   Markus Nix <mnix@docuverse.de>
 */
class XML_MXML_Containers_Grid extends XML_MXML_Containers_Box
{
    // {{{ public properties
    /**
     * Element name.
     ...
     * @return  object  XML_MXML_Containers_GridRow
     * @access  public
     * @throws  Exception
     */
    public function addRow($item = array())
    {
        if (!is_object($item)) {
            try {
                $item = $this->getDocument()->createElement('gridrow', $item);
            } catch (Exception $e) {
                // rethrow
                throw $e;
            }
