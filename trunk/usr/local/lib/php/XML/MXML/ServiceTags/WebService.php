 ...
 * @author   Markus Nix <mnix@docuverse.de>
 */
class XML_MXML_ServiceTags_WebService extends XML_MXML_Element
{
    // {{{ public properties
    /**
     * Element name.
     ...
     * @return  object  XML_MXML_ServiceTags_Operation
     * @access  public
     * @throws  Exception
     */
    public function addOperation($item = array())
    {
        if (!is_object($item)) {
            try {
                $item = $this->getDocument()->createElement('operation', $item);
            } catch (Exception $e) {
                // rethrow
                throw $e;
            }
