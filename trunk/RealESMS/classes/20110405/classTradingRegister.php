			...
			if($this->_ERROR_CODE == 0)
			{
				...
				if($Deposit['ErrorCode']=='0')
				{
					...
					if($dab_rs == 0){
						...
						$soap = &new Bravo();
						$Deposit_value = array("TradingDate" => $Today, 'TransactionType'=>$TransactionType, "AccountNo" => $Deposit['AccountNo'], "Amount" => $Deposit['Amount'], "Bank"=> $BravoCode, "Branch"=> "", "Note" => "Nhan co tuc chuyen doi tu cp le ".$Deposit['StockSymbol']); //'011C001458'
						//var_dump($withdraw_value);
						$ret = $soap->deposit($Deposit_value);
						if($ret['table0']['Result']==1){
