...
class CBank extends WS_Class{
	...
	function CBank($check_ip) {
		...
		$arr = array(
					...
					'getTransactionList' => array(
										...
										'output' => array( 'ID', 'AccountID', 'AccountNo', 'InvestorName', 'Amount', 'IsConfirmed', 'TransactionStatus', 'TransTypeName', 'TransTypeID', 'BankID', 'BankName', 'TradingDate', 'CreatedBy', 'UpdatedBy', 'TransactionBankID', 'TransactionBankName', 'CreatedDate', 'UpdatedDate', 'Note' )
										),
					...
					'cofirmDeposit' => array(
										'input' => array( 'ID', 'UpdatedBy', 'TransactionBankID', 'AccountNo', 'Amount', 'Note' ),
										'output' => array()
										),
	...
	*/
	...
	function cofirmDeposit($ID, $UpdatedBy, $TransactionBankID, $AccountNo, $Amount, $Note="") {
		...
		if ( !required($ID) ) {
			...
			if (empty( $rs)) {
				...
				$result = $rs['varerror'];
				$content = date("d/m/Y H:i:s") ."	Nop: ID => $ID, AccountNo => $AccountNo, Amount => $Amount, UpdatedBy => $UpdatedBy --> $result" ;
	...
	function cofirmWithdrawal($ID, $UpdatedBy, $TransactionBankID, $AccountNo, $Amount, $Note="") {
		...
		if ( !required($ID) ) {
			...
			if (empty( $rs)) {
				...
				$content = date("d/m/Y H:i:s") ."	Rut: ID => $ID, AccountNo => $AccountNo, Amount => $Amount, UpdatedBy => $UpdatedBy --> $result" ;
				write_log($function_name, $content, VIRTUAL_BANK_LOG_FILE_PATH) ;
