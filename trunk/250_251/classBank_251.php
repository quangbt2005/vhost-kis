	...
	function CBank($check_ip) {
		...
		$arr = array(
					...
					'getAccountBankInfo' => array(
										...
										'output' => array( 'AccountID', 'BankID', 'BankAccount', 'Amount', 'LockAmount', 'UsableAmount', 'Debt', 'DebtInDue' )
										),
					'insertDeposit' => array(
					...
					'getBankListForWD' => array(
										'input' => array(),
                    'output' => array( 'BankID', 'BankName', 'ShortName', 'BravoCode' )
										),
	...
	function getAccountBankInfo($AccountID, $BankID) {
		...
		if($num_row>0) {
			switch ($result['bankid']) {
				case DAB_ID:
					$dab = &new CDAB();
					$usable = $dab->getAvailBalance($result['bankaccount'], $AccountNo);
					break;
				...
				case NVB_ID:
					$nvb = &new CNVB();
					$usable = $nvb->getAvailBalance(time(), $result['bankaccount']);
					break;
            ...
            'Debt'  => new SOAP_Value("Debt", "string", $result['debt'] ),
            'DebtInDue'  => new SOAP_Value("DebtInDue", "string", $result['debtindue'] ),
						)
				);
		}
		return returnXML(func_get_args(), $this->class_name, $function_name, $this->_ERROR_CODE, $this->items, $this );
      ...
      if (empty( $rs)) {
        ...
        if ($result < 0) {
          ...
          if(!empty($acc_rs['mobilephone'])){
            $message  = 'Tai khoan cua quy khach tai KIS da thay doi: %2B' . number_format( $Amount, 0, '.', ',' ) . '. '.$Note;
