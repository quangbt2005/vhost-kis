...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
          ...
          'checkQuotaAccountIsExisted' => array( 'input' => array( 'AccountNo' ),
                  'output' => array( 'Boolean' )),
            );
  ...
  */
  function updateBuyOrder($OrderID, $OrderQuantity, $OrderPrice, $OrderStyleID, $AccountNo, $UpdatedBy){
    ...
    if ( !required($OrderID) || !unsigned($OrderID) || !required($OrderQuantity) || !unsigned($OrderQuantity) || !required($OrderPrice) ) {
      ...
      if ( ($bank_rs['bankaccount'] != "") || ($bank_rs['bankid'] == NHHM) ) {
      ...
      if ( ($bank_rs['bankaccount'] != "") || ($vip == 1) || ($bank_rs['bankid'] == NHHM) ) {
        if ( strpos (PAGODA_ACCOUNT, $AccountNo) === false && $vip == 0) {
          ...
          switch ($bank_rs['bankid']) {
            ...
            case OFFLINE:
              ...
              $query = sprintf( "CALL sp_VirtualBank_Edit( '%s', %u, %u, %f, '%s')", $AccountNo, $bank_rs['bankid'], $OrderID, $orderValue, $UpdatedBy);
        ...
        if ( $rs['ordersideid'] == ORDER_BUY && (strpos (PAGODA_ACCOUNT, $AccountNo) === false) && $vip == 0) {
          ...
          switch ($bank_rs['bankid']){
            ...
            case OFFLINE:
              ...
              $query = sprintf( "CALL sp_VirtualBank_Cancel('%s', %u, %u, %f, '%s')", $AccountNo, $bank_rs['bankid'], $OrderID, $orderValue, $UpdatedBy);
              $this->_MDB2_WRITE->connect();
              $off_rs = $this->_MDB2_WRITE->extended->getRow($query);
              $this->_MDB2_WRITE->disconnect();
              $dab_rs = $off_rs['varerror'];
      ...
      if ( $this->_ERROR_CODE == 0 ) {
        ...
        if (empty( $rs)) {
          ...
          if ($result < 0) {
            ...
            if ( strpos (PAGODA_ACCOUNT, $AccountNo) === false && $vip == 0) {
              $dab_rs = 999;
              if($isQuotaAccount != 0){  // Neu khach hang su dung dich vu T2 thi chi giao dich voi bank NHHM
                $BankID = NHHM;
                $mdb    = initWriteDB();
            ...
            } else { // lock money fail
              if($isQuotaAccount != 0){ // ==> BankID = NHHM
                switch ($dab_rs) {
                  case '-1':
          ...
          if ($result < 0) {
            ...
            if ( strpos (PAGODA_ACCOUNT, $AccountNo) === false && $vip == 0) {
              ...
              if($isQuotaAccount != 0){  // Neu khach hang su dung dich vu T2 thi chi giao dich voi bank NHHM
                ...
                for($i=0; $i<count($bank_rs); $i++) {
                  if(in_array($bank_rs[$i]['bankid'], array(DAB_ID, VCB_ID, ANZ_ID, NVB_ID, OFFLINE))){
                    switch ($bank_rs[$i]['bankid']) {
                      case DAB_ID:
                        $dab = &new CDAB();
                        $dab_rs = $dab->blockMoney($bank_rs[$i]['bankaccount'], $bank_rs[$i]['cardno'], $AccountNo, $result, $rs['varordervalue'], $OrderDate);
                    ...
                    $last_bank = $bank_rs[$i]['bankid'];
                    $last_bank_result = $dab_rs;
                  } // if in_array
                  if ($last_bank_result == 0){
                    $BankID = $last_bank;
                    $dab_rs = $last_bank_result;
                    break; // break for
                  }
                } // end for
              } // else not NHHM
            ...
            if ($dab_rs == 0) { //Successfully
              ...
              //$mdb->connect();
              $mdb = initWriteDB();
              $query = sprintf( "CALL sp_updateBuyingOrderFromPendingToDenied(%u, '%s') ", $result, 'Last Bank ID: ' . $last_bank . ', Error Code:' . $dab_rs );
              $status_rs = $mdb->extended->getRow($query);
              $mdb->disconnect();
          ...
          if ($orderValue > 0) {
            ...
            switch ($bank_rs['varbankid']) {
              ...
              case OFFLINE:
                //inAccountNo varchar(20),inBankID int,inOrderID bigint,inOrderAmount double,inCreatedBy  $OrderID, $AccountNo, $orderValue,
                $mdb = initWriteDB();
                $query = sprintf( "CALL sp_VirtualBank_Edit( '%s', %u, %u, %f, '%s')", $AccountNo, $bank_rs['varbankid'], $OrderID, $orderValue, $UpdatedBy);
                $off_rs = $mdb->extended->getRow($query);
                $mdb->disconnect();
                $dab_rs = $off_rs['varerror'];
                break;
    ...
    if ( !required($OrderNumber) || !unsigned($OrderNumber) || !required($CancelQuantity) || !unsigned($CancelQuantity) || !required($OrderDate) ) {
      ...
      if ($rs['varorderid'] > 0 ) {
        switch ($rs['varbankid']) {
          ...
          case OFFLINE:
            //inAccountNo varchar(20),inBankID int,inOrderID bigint,inCancelAmount double,inCreatedBy
            $query = sprintf( "CALL sp_VirtualBank_Cancel('%s', %u, %u, %f, '%s')", $rs['varaccountno'], $rs['varbankid'], $rs['varorderid'], $rs['varcancelvalue'] , $function_name);
            $this->_MDB2_WRITE->connect();
            $off_rs = $this->_MDB2_WRITE->extended->getRow($query);
            $this->_MDB2_WRITE->disconnect();
            $dab_rs = $off_rs['varerror'];
...
return true;
  ...
  */
  ...
  // End 20100723 - Them $StockExchangeID ------------------------------------------------------- //
    ...
    if ( !required($OrderDate) ) {
        ...
        if ( strpos (PAGODA_ACCOUNT, $rs[$i]['accountno']) === false && $vip == 0) {
          if ($rs[$i]['ordersideid'] == ORDER_BUY) {
            switch ($rs[$i]['bankid']){
              ...
              case NHHM:
              case OFFLINE:
                $query = sprintf( "CALL sp_VirtualBank_Cancel('%s', %u, %u, %f, '%s')", $rs[$i]['accountno'], $rs[$i]['bankid'], $rs[$i]['id'], $rs[$i]['amount'], $function_name);
                $this->_MDB2_WRITE->connect();
                $off_rs = $this->_MDB2_WRITE->extended->getRow($query);
                $this->_MDB2_WRITE->disconnect();
                $dab_rs = $off_rs['varerror'];
      ...
      mailSMTP('Qu?n lý Ð?t l?nh','webmaster@eps.com.vn','quan.l@eps.com.vn','ba.nd@eps.com.vn', '', 'Danh sách l?nh d?t tru?c không h?p l? - StockExchangeID:'.$StockExchangeID,'Danh sách l?nh d?t tru?c không h?p l? \r\n <br>'.$bank_result);
      // mailSMTP('Qu?n lý Ð?t l?nh','webmaster@eps.com.vn','quang.tm@eps.com.vn','quang.tm@eps.com.vn', '', 'Danh sách l?nh d?t tru?c không h?p l? - StockExchangeID:'.$StockExchangeID,'Danh sách l?nh d?t tru?c không h?p l? \r\n <br>'.$bank_result);
          ...
          if ( strpos (PAGODA_ACCOUNT, $AccountNo) === false && $vip == 0) {
            ...
            if ($rs['varreblockedvalue'] > 0) {
              switch ($rs['varbankid']) {
                ...
                case NHHM:
                case OFFLINE:
                  $query = sprintf( "CALL sp_VirtualBank_Edit( '%s', %u, %u, %f, '%s')", $rs['varaccountno'], $rs['varbankid'], $OrderID, $rs['varreblockedvalue'], $UpdatedBy);
                  $this->_MDB2_WRITE->connect();
                  $off_rs = $this->_MDB2_WRITE->extended->getRow($query);
                  $this->_MDB2_WRITE->disconnect();
                  $dab_rs = $off_rs['varerror'];
  ...
  */
  ...
  function insertHOSE_MP_BuyingOrder($AccountNo, $StockID, $OrderQuantity, $Session, $FromType, $Note, $OrderDate, $IsAssigner, $CreatedBy){
