...
define("WS_ORDER", "http://172.25.2.250/ws/order.php?wsdl");
				...
				if ($at_rs['varCancelValue'] > 0 ) {
					if ($at_rs['varOrderID'] > 0 ) {
						switch ($at_rs['varBankID']) {
							...
							case OFFLINE:
								$query = sprintf( "CALL sp_VirtualBank_Cancel('%s', %u, %u, %f, '%s')", $at_rs['varAccountNo'], OFFLINE, $at_rs['varOrderID'], $at_rs['varCancelValue'], 'HOET');
								$this->_MDB2_WRITE->connect();
							...
							if ( strpos (PAGODA_ACCOUNT, $order_rs['AccountNo']) === false ) {
								switch($order_rs['BankID']) {
									...
									case VCB_ID:
										...
										if ($newAmount > 0)
											$dab_rs = $dab->editBlockMoney( $order_rs['AccountNo'], $oldOrderID, $newOrderID, $order_rs['OldOrderValue'], $newAmount, $function_name );
										else
											$dab_rs = $dab->cancelBlockMoney($order_rs['AccountNo'], $oldOrderID, $order_rs['OldOrderValue'] );
										break;
										
									case OFFLINE:
										$query = sprintf( "CALL sp_VirtualBank_Cancel('%s', %u, %u, %f, '%s')", $order_rs['AccountNo'], OFFLINE, $order_rs['OldOrderID'], $order_rs['OrderValue'], 'HOET');
										$this->_MDB2_WRITE->connect();
										$off_rs = $this->_MDB2_WRITE->extended->getRow($query);
										$this->_MDB2_WRITE->disconnect();
										$dab_rs = 0;
	...
	*/
    function insertGA($MessageType, $AdminMessageLengh, $AdminMessageText) {
		...
		try{
			...
			another_write_log($function_name, $content, 'directly_tranfer');
			//newSendSMS('0976149240,0933886911', $AdminMessageText);
					...
					if ( strpos (PAGODA_ACCOUNT, $AccountNo) === false) {
						...
						if ($rs['varReBlockedValue'] > 0) {
							switch ($rs['varBankID']) {
								...
								case OFFLINE:
									$query = sprintf( "CALL sp_VirtualBank_Edit( '%s', %u, %u, %f, '%s')", $rs['varAccountNo'], OFFLINE, $OrderID, $rs['varReBlockedValue'], 'HNX');
									$this->_MDB2_WRITE->connect();
									$off_rs = $this->_MDB2_WRITE->extended->getRow($query);
									$this->_MDB2_WRITE->disconnect();
			...
			if ( strpos (PAGODA_ACCOUNT, $AccountNo) === false ) {
				switch($BankID) {
					...
					case VCB_ID:
						...
						if ($newAmount > 0)
							$dab_rs = $dab->editBlockMoney( $AccountNo, $oldOrderID, $newOrderID, $OldOrderValue, $newAmount, $function_name );
						else
							$dab_rs = $dab->cancelBlockMoney($AccountNo, $oldOrderID, $OldOrderValue );
						break;
						
					case OFFLINE:
						$query = sprintf( "CALL sp_VirtualBank_Cancel('%s', %u, %u, %f, '%s')", $AccountNo, OFFLINE, $OldOrderID, $OrderValue, 'HNX');
						$this->_MDB2_WRITE = newInitWriteDB();
						$off_rs = $this->_MDB2_WRITE->extended->getRow($query);
						$content = date("d/m/Y H:i:s") ."	". $query ."	--> ". $off_rs['varError'];
						another_write_log($function_name, $content, 'directly_tranfer');
