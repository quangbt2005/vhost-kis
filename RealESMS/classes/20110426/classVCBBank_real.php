...
class CVCBBank extends WS_Class{
	...
	var $_ERROR_CODE;
	var $items;
	var $path =  '/home/vhosts/eSMSstorage/bank/';
  ...
  function BuyAndVSDFeeFile()
  {
    $class_name = $this->class_name;
    $function_name = 'BuyAndVSDFeeFile';
    ...
    if(file_exists($filepath)===true)
    {
      ...
      $query = sprintf("CALL sp_VCB_getAllBuyingOrder('%s')",date('Y-m-d'));
      $this->_MDB2->connect();
      $BuyList = $this->_MDB2->extended->getAll($query);
      $this->_MDB2->disconnect();
      $BuyCount = count($BuyList);

      $date_array['year'] = '2009';
      $date_array['mon'] = '05';

      $query = sprintf("Call sp_FeeForVSD_getListForVCBAuction('%s','%s')",$date_array['year'], $date_array['mon']);
      $this->_MDB2->connect();
      $VSDFeeList = $this->_MDB2->extended->getAll($query);
      $this->_MDB2->disconnect();
      $VSDFeeCount = count($VSDFeeList);

      // ------------------------------------------------------------------------------------------
      // Phai luu lai refno cua VSDFee de khi lay file
      //  ket qua ve moi so sanh lai voi file log nay
      //  de biet cai nao la buy, cai nao la VSDFee
      $vsdrefno = '';
      foreach($VSDFeeList as $value){
        if(!empty($value['orderid'])){
      	  $vsdrefno .= ('_' . $value['orderid']);
        }
      }
      $vsdrefno = trim($vsdrefno,'_');
      $vsdlogfile = 'vsdrefno_' . date('Ymd') . '.txt';
      $vsdlogfile = $csv_dir.$vsdlogfile;

      $vfp = @fopen($vsdlogfile,'w');
      flock($vfp, LOCK_EX);
      fwrite($vfp, $vsdrefno);
      flock($vfp, LOCK_UN);
      fclose($vfp);
      @chmod($vsdlogfile, 0644);
      // ------------------------------------------------------------------------------------------

      $List = array_merge($BuyList, $VSDFeeList);
      $num_count = $BuyCount + $VSDFeeCount;
      ...
      if($num_count>0){
        ...
        for ($i=0; $i<$num_count; $i++) {
          if(!empty($List[$i]['orderid'])){
            $message .= $List[$i]['orderid'].";";
            $message .= $List[$i]['makhop'].";";
            $message .= $List[$i]['orderstockstatus'].";";
            $message .= $List[$i]['accountno'].";";
            $message .= $List[$i]['symbol'].";";
            $message .= $List[$i]['price'].";";
            $message .= "B;";
            $message .= $List[$i]['orderdate'].";";
            $message .= $List[$i]['loaitien'].";";
            $message .= $List[$i]['chinhanh'].";";
            $message .= $List[$i]['ngonngu'].";";
            $message .= $List[$i]['tong'].";";
            $message .= $List[$i]['loailenh'].";\n";
          }
        }
        //print_r($message);
        flock($fp, LOCK_EX);
        ...
        while(!feof($handle)){
          ...
          if(in_array($splited[1], $vsdrefarray)){
            ...
            $this->items[] = new SOAP_Value(
                ...
                array(
                  'Refno'      => new SOAP_Value("Refno", "string", $splited[1]),
                  'OrderID'    => new SOAP_Value("OrderID", "string", $orderID),
                  'UnitCode'   => new SOAP_Value("UnitCode", "string", $unitCode),
                  'AccountNo'  => new SOAP_Value("AccountNo", "string", $splited[4]),
                  'Amount'     => new SOAP_Value("Amount", "string", $splited[12]),
